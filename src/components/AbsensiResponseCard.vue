<template>
  <div>
    <div class="input-area">
      <button
        v-if="userImage != undefined && userVideo != undefined"
        @click="LivenessCheck"
        class="col-sm-12"
      ...
      axios
        ...
        .then((response) => {
          ...
          this.responseData.confidenceLevel =
            response.data.verification.confidence_level;
        ...
        .catch((error) => {
          console.log(error);
          this.responseData.timeDataFrontend =
            (new Date().getTime() - error.config.meta.requestStartedAt) / 1000;
