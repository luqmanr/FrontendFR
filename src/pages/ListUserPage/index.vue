  ...
  <div>
    <div
      style="
        ...
        background-color: rgba(18, 18, 139, 0.466);
      "
    >
      <p style="color: white">
        Isi data-data berikut untuk request list User yang terdaftar
      </p>
    </div>
    ...
    <div class="row user-data">
      ...
      <div class="col-md-6 row">
        ...
        <input
          type="text"
          v-model="payloadData.accessToken"
          class="col-sm-11"
        />
      </div>
    </div>
...
export default {
  data() {
    return {
      ...
      // listUserAPI: "https://api.riset.ai/api_fr/v2/listface",
      payloadData: {
        region: "Trial",
        accessToken: "",
        trx_id: "",
  ...
  methods: {
    listUser() {
      const listUserPayload = {
        ...
        cluster_id: this.payloadData.region,
        cluster_key: this.payloadData.accessToken,
    ...
    makeTrx_id() {
      // generate random string, seperti ini "ck5bec23-51f5-4c6e-aca5-ebd7115a8f40" / "8s-4s-4s-4s-12s"
      var characters =
        "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
      var charactersLength = characters.length;
      var eight = "";
      for (var i = 0; i < 8; i++) {
        eight += characters.charAt(
          Math.floor(Math.random() * charactersLength)
        );
      }
      var four_one = "";
      for (var i = 0; i < 4; i++) {
        four_one += characters.charAt(
          Math.floor(Math.random() * charactersLength)
        );
      }
      var four_two = "";
      for (var i = 0; i < 4; i++) {
        four_two += characters.charAt(
          Math.floor(Math.random() * charactersLength)
        );
      }
      var four_three = "";
      for (var i = 0; i < 4; i++) {
        four_three += characters.charAt(
          Math.floor(Math.random() * charactersLength)
        );
      }
      var twelve = "";
      for (var i = 0; i < 12; i++) {
        twelve += characters.charAt(
          Math.floor(Math.random() * charactersLength)
        );
      }
      var trx_id = "".concat(
    ...
    makeTimestamp() {
      // generate timestamp dengan format seperti ini "2020-05-13 09:43:26+0700" / "yyyy-mm-dd hh:mm:ss+tttt"
