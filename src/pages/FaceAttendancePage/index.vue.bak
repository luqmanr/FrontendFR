           ...
           <!-- INI ISINYA PREVIEW IMG dari object "displayObjects.input_image",
            ...
            <div class="photo-card col-md-6">
                <div class="photo-section">
                    ...
                    <attendance-card @userPhoto="$_AttendanceCard_ReceiveImage"></attendance-card>
...
export default {
    ...
    methods: {
        ...
        logAbsensiLog() {
            ...
            var query = (
                client_id + "&" + user_name + "&" + 
                user_id + "&" + confidence_level + "&" + trx_id + "&" + timestamp
                )
            
            var i
            for (i = 0; i < apiEndPoints.length; i++) {
                console.log(apiEndPoints[i] + query)
                axios.get(apiEndPoints[i] + query)
                  .then((response) => {
                    console.log(response)
                  })
            }
